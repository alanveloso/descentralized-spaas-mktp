...
describe("SpectrumProviderManager", function () {
  ...
  it("Should allow provider to supply tokens", async function () {
    await providerManager.connect(provider).provideTokens(provider.address, 1, 50, ethers.parseEther("0.01"));
  ...
  it("Should fail to supply tokens with zero amount", async function () {
    await expect(
      providerManager.connect(provider).provideTokens(provider.address, 1, 0, ethers.parseEther("0.01"))
    ).to.be.revertedWith("Amount must be greater than zero");
  });

  it("Should fail to supply tokens with zero rental rate", async function () {
    await expect(
      providerManager.connect(provider).provideTokens(provider.address, 1, 50, ethers.parseEther("0"))
  ...
  it("Should update provider's token balance correctly after multiple supplies", async function () {
    await providerManager.connect(provider).provideTokens(provider.address, 1, 50, ethers.parseEther("0.01"));
    await providerManager.connect(provider).provideTokens(provider.address, 1, 25, ethers.parseEther("0.01"));
    const balance = await providerManager.providerTokenBalances(provider.address, 1);
    expect(balance).to.equal(75);
  });

  it("Should list providers sorted by rental rate", async function () {
    await providerManager.connect(provider).provideTokens(provider.address, 1, 50, ethers.parseEther("0.02"));
    await providerManager.connect(provider2).provideTokens(provider2.address, 1, 50, ethers.parseEther("0.01"));
  ...
  it("Should revert if no provider has enough tokens for the requested amount", async function () {
    await providerManager.connect(provider).provideTokens(provider.address, 1, 20, ethers.parseEther("0.01"));
    await providerManager.connect(provider2).provideTokens(provider2.address, 1, 15, ethers.parseEther("0.02"));
  ...
  it("Should update provider's balance after allocation", async function () {
    ...
    await providerManager.connect(provider).provideTokens(provider.address, 1, 100, ethers.parseEther("0.01"));
